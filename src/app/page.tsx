...
import { UploadCloud, FileVideo, FileImage, Loader2, Download, RefreshCw, Settings, Sun, Moon, Trash2 } from 'lucide-react';
...
export default function Home() {
  ...
  const convertFiles = async () => {
    if (files.length === 0 || !ffmpegRef.current || !isReady || isConverting) return;
    ...
    try {
      if (!ffmpeg.loaded) {
        console.log("Loading standard FFmpeg core...");
        await ffmpeg.load({
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        setFiles(prev => prev.map(f => f.id === currentFile.id ? { ...f, status: 'converting', progress: 0 } : f));

        ffmpeg.on('progress', ({ progress }: any) => {
          if (progress >= 0 && progress <= 1) {
            setFiles(prev => prev.map(f => 
              f.id === currentFile.id ? { ...f, progress: Math.round(progress * 100) } : f
            ));
          }
        });

        try {
          const safeInputName = 'input_' + i + currentFile.file.name.substring(currentFile.file.name.lastIndexOf('.'));
          const outputName = `converted_${i}.${outputFormat}`;

          await ffmpeg.writeFile(safeInputName, await fetchFile(currentFile.file));
          
          const args = ['-i', safeInputName, outputName];
        ...
        // Unbind progress event for the next file
        ffmpeg.off('progress');
      }

    } catch (error) {
      console.error("FFmpeg initialization error:", error);
      alert("Error initializing FFmpeg. Check console for details.");
    } finally {
      ...
      <div className="max-w-3xl w-full space-y-10">
        ...
        <div className="text-center space-y-4">
          ...
          <p className="text-lg text-slate-600 dark:text-slate-400 max-w-xl mx-auto leading-relaxed transition-colors">
            Fast, private, and free. Convert multiple images and videos directly in your browser without uploading to any servers.
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900/80 backdrop-blur-sm p-8 md:p-10 rounded-3xl shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-800 transition-all">
          ...
          <div 
            className={`relative border-2 border-dashed rounded-2xl p-8 md:p-12 text-center transition-all duration-200 ease-in-out mb-6 ${
              isDragActive 
          ...
          >
            ...
            <div className="flex flex-col items-center text-slate-500 dark:text-slate-400 pointer-events-none">
              ...
              <span className="font-semibold text-lg text-slate-700 dark:text-slate-300">Drag & drop your files here</span>
          ...
          {files.length > 0 && (
            <div className="space-y-3 mb-8 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
              {files.map(fileItem => (
                <div key={fileItem.id} className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700/50">
                  <div className="flex items-center space-x-4 overflow-hidden">
                    {fileItem.file.type.startsWith('video') ? <FileVideo className="w-8 h-8 text-indigo-500 flex-shrink-0" /> : <FileImage className="w-8 h-8 text-indigo-500 flex-shrink-0" />}
                    <div className="flex flex-col min-w-0">
                      <span className="font-medium text-slate-800 dark:text-slate-200 truncate">{fileItem.file.name}</span>
                      <span className="text-xs text-slate-500 dark:text-slate-400">{(fileItem.file.size / 1024 / 1024).toFixed(2)} MB</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center space-x-4 flex-shrink-0 ml-4">
                    ...
                    {fileItem.status === 'idle' && (
                      <button onClick={() => removeFile(fileItem.id)} disabled={isConverting} className="text-slate-400 hover:text-red-500 transition-colors disabled:opacity-50" aria-label="Remove file">
          ...
          <div className="flex flex-col md:flex-row gap-6 items-end border-t border-slate-100 dark:border-slate-800 pt-6">
            ...
            <button
              ...
              disabled={files.length === 0 || isConverting || pendingCount === 0}
